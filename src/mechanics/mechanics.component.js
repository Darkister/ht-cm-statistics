...
function fillMechanics(){
  var mechanicsRange = mechanicSheet.getRange(1,2,37,30),
      ...
      avgFailsMinPhases = ["Jormag","Jormag","Primordus","Kralkatorrik","Mordremoth","Purification 3","Soo-Won 1","Soo-Won 1","Purification 1"],
      value = 0;
  ...
  for(var j = 0; j < 3; j++){
    ...
    for(var i = 0; i < 10; i++){
      ...
      for(var n = 0; n <= mechanicsToCheck.length; n++){
        mechanicsValue[n + 2 + (j * 13)][1 + (i * 3)] = getAmountOfMechanicFailes(logSheet.getRange(2,n+19,logSheet.getLastRow() - 1,1).getValues(),player,days);
        mechanicsValue[n + 2 + (j * 13)][0 + (i * 3)] = avgFailsPerTry(player,mechanicsValue[n + 2 + (j * 13)][1 + (i * 3)],avgFailsMinPhases[n],days);
        value += 2;
        if(j>0){
          mechanicsValue[n + 2 + (j * 13)][2 + (i * 3)] = mechanicsValue[n + 2 + (j * 13)][0 + (i * 3)] < mechanicsValue[n + 2 + ((j - 1) * 13)][0 + (i * 3)] ? "+" : mechanicsValue[n + 2 + (j * 13)][0 + (i * 3)] == mechanicsValue[n + 2 + ((j - 1) * 13)][0 + (i * 3)] ? "~" : mechanicsValue[n + 2 + (j * 13)][1 + (i * 3)] == mechanicsValue[n + 2 + ((j - 1) * 13)][1 + (i * 3)] ? "~" : "-";
          value++;
        }
        mechanicsSheet.getRange(41,1,1,1).setValues([["in progress ... " + String(+(Math.round(value/720*100 + "e+2") + "e-2")) + " %"]]);
      }
...
function avgFailsPerTry(player,totalValue,phase,days=-1){
  ...
  if(days==-1){
    var filteredRows = phases.filter((row, i) =>
      allowedPhases.includes(row[0]) && players[i].includes(player)
    );
