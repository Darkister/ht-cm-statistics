...
function createStatisticsLayout(){
    ...
    statisticsValue[4][1] = "=SUMME(MAP(U10:U; S10:S;LAMBDA(valA;valB;MIN(valA;valB))))/R9";
...
function cleanUpStatisticsLayout(){
    var statisticsRange = statisticsSheet.getRange(9,1,statisticsSheet.getLastRow()-8,5),
        statisticsValue = statisticsRange.getValues();

    for(var i = 1; i < statisticsValue.length; i++){
        if(statisticsValue[i][1] == 0){
            statisticsSheet.getRange(i+9,1,1,5).setValues([["","","","",""]])
                .setBorder(false,false,false,false,false,false);
        }
    }
    statisticsSheet.getRange(9,1,statisticsSheet.getLastRow()-8,5)
